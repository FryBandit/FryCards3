...
const BattleArena: React.FC = () => {
  ...
  const startBattle = (deck: Deck) => {
    setSelectedDeck(deck);
    setBattle({
      playerHP: 100,
      opponentHP: 100,
      turn: 'player',
      log: [`BATTLE START: ${deck.name} VS SYSTEM_GUARDIAN`],
      isFinished: false
    });
    setGameState('battle');
  };
  ...
  const executeTurn = (playerMove: boolean) => {
    ...
    setBattle(prev => {
      ...
      const nextLog = [...prev.log, 
        `${isPlayerTurn ? 'PLAYER' : 'GUARDIAN'} DEPLOYS ${crit ? 'CRITICAL ' : ''}ATTACK: ${finalDamage} DMG`
      ];
      ...
      if (finished) {
        nextLog.push(`MATCH ENDED: ${winner === 'player' ? 'VICTORY' : 'DEFEAT'}`);
      }
  ...
  // Auto opponent turn
  useEffect(() => {
    if (battle.turn === 'opponent' && !battle.isFinished && gameState === 'battle') {
      const timer = setTimeout(() => executeTurn(false), 1200);
      return () => clearTimeout(timer);
    }
  }, [battle.turn, battle.isFinished, gameState]);
    ...
    return (
      <div className="max-w-4xl mx-auto py-10 animate-fade-in">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-500 font-mono text-sm uppercase tracking-widest">Select your combat squad and initiate engagement.</p>
        </div>
        ...
        {userDecks.length === 0 ? (
          <div className="glass p-12 text-center rounded-sm border-2 border-slate-800">
             ...
             <p className="text-slate-500 font-bold mb-6 uppercase tracking-widest font-mono text-xs">NO COMBAT SQUADS DETECTED IN YOUR SYSTEM</p>
              ...
              >
                ...
                <div className="relative z-10">
                   ...
                   <div className="flex gap-2 mb-6">
                      <div className="px-3 py-1 bg-slate-950 border border-slate-800 text-[10px] font-black text-indigo-400">5 CARDS</div>
                      <div className="px-3 py-1 bg-slate-950 border border-slate-800 text-[10px] font-black text-slate-500 uppercase tracking-widest font-mono">STANDBY</div>
                   </div>
                   <div className="flex items-center gap-2 text-indigo-500 font-heading text-[10px] group-hover:translate-x-2 transition-transform">
                     INITIATE ENGAGEMENT <ChevronRight size={14} />
                   </div>
                </div>
              </button>
            ))}
          ...
          <div className="lg:col-span-4 flex flex-col bg-slate-950/80 border-2 border-slate-800 rounded-sm overflow-hidden">
             ...
             <div className="flex-1 overflow-y-auto p-4 font-mono text-[11px] space-y-2 custom-scrollbar">
                {battle.log.map((entry, i) => (
                  <div key={i} className={`p-2 rounded-sm border-l-2 ${entry.includes('VICTORY') ? 'border-green-500 text-green-400 bg-green-500/5' : entry.includes('DEFEAT') ? 'border-red-500 text-red-400 bg-red-500/5' : 'border-indigo-500/30 text-slate-400'}`}>
          ...
          <div className="lg:col-span-4 flex flex-col justify-between">
             <div className="space-y-4">
                <div className="flex justify-between items-end">
                   ...
                   <h2 className="font-heading text-xs text-white uppercase tracking-widest">SYSTEM_GUARDIAN</h2>
                </div>
                <div className="h-6 w-full bg-slate-950 border-2 border-slate-800 p-1 rounded-sm overflow-hidden">
                   <motion.div 
             ...
             <div className="bg-slate-950/50 border border-slate-800 p-4 rounded-sm text-center font-mono text-[10px] text-slate-600 uppercase tracking-[0.3em]">
                {battle.isFinished ? 'SYSTEMS OFFLINE' : 'GUARDIAN PROTOCOLS ACTIVE'}
             </div>
          </div>
       </div>
           ...
           >
              <div className={`max-w-md w-full p-12 rounded-sm border-2 text-center shadow-2xl ${battle.winner === 'player' ? 'border-green-500 bg-green-500/10' : 'border-red-500 bg-red-500/10'}`}>
                 ...
                 <div className="flex flex-col gap-4">
                    <button 
                      onClick={() => startBattle(selectedDeck!)}
